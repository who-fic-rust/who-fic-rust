...
- `Class` has a `code` attribute (the classification code) and a `kind`
  attribute (e.g. `chapter`, `block`, `category` — open-ended per the
  file's own `ClassKinds` declaration; treat as a string).
- `SuperClass code="..."` references the parent `Class` by code. A `Class`
  ...
  — the parser should collect all `SuperClass` elements present, not just
  the first.
...
- ICD-10-specific `ModifierClass`/`Modifier` elements (used for shared
  ...
  `code` and contains `Modifier code="...">` entries with their own
  `Rubric`s) without attempting to model ICD-10's specific modifier
  semantics — that belongs in `who-fic-icd` if ever implemented.

## Types

- `ClamlDocument` — top-level parsed document: `title() -> Option<&str>`,
  `classes() -> &[Class]`, `modifier_classes() -> &[ModifierClass]`.
  Parsed via `ClamlDocument::from_str(&str)` /
  `ClamlDocument::from_reader(impl std::io::Read)`.
...
- `Label` — `lang() -> &str` (from `xml:lang`, defaulting to `"en"` if
  ...
  concatenated text content and does not preserve markup structure —
  document this as a deliberate simplification.
- `ModifierClass` / `Modifier` — generic form per the format section above.
- `ClamlError` — `std::error::Error + Display + Debug`, wraps the
  underlying XML parse error plus WHO-FIC-specific structural problems
  (e.g. a `Class` with no `code` attribute), reporting position
  information from the underlying XML parser where available.
...
a good fit for a hand-written parser. `quick-xml` becomes this crate's one
required dependency (the rest of the workspace stays dependency-free by
design; this is the deliberate, documented exception because the format
itself demands a real XML parser).
...
tag), a `Class` missing its `code` attribute.
